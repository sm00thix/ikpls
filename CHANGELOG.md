...
- **Breaking:** The per-sample weights parameter was renamed from `weights` to `sample_weight` across the entire public API -- `fit`, `stateless_fit`, and `cross_validate` on the NumPy (`ikpls.numpy`) and JAX (`ikpls.jax`) PLS classes, and `cross_validate` on both fast cross-validation classes (`ikpls.fast_cross_validation.numpy` / `.jax`) -- to match scikit-learn's `sample_weight` naming convention (already used by the `ikpls.sklearn.PLS` wrapper). Callers that passed `weights=` by keyword must switch to `sample_weight=`; positional calls are unaffected.
