...
- **Breaking:** The JAX PLS implementations no longer emit the "weight is close to zero" underflow warning, and `max_stable_components` is always `None` for them. That warning relied on an ordered host `io_callback` that is incompatible with `jax.vmap` and runs counter to JAX's fast, batched execution model; the JAX fit now runs warning-free and is therefore directly `jax.vmap`-able (e.g. to batch many fits). The NumPy implementations are unchanged — they still emit the warning and set `max_stable_components`.
