"""
This script demonstrates the usage of the NumPy implementation of IKPLS for fitting on
...
if __name__ == "__main__":
    ...
    # These two lines are equivalent to calling np_ikpls_alg_1.fit_transform(X=X, Y=Y, A=A) or np_ikpls_alg_1.fit(X=X, Y=Y, A=A).transform(X=X, Y=Y)
    np_ikpls_alg_1.fit(X=X, Y=Y, A=A)
    X_scores_using_all_components, Y_scores_using_all_components = (
        np_ikpls_alg_1.transform(X=X, Y=Y, n_components=None)
    )  # Shapes are (N, A) = (100, 20).

    also_X_scores_using_all_components = np_ikpls_alg_1.transform(
        X=X
    )  # Also works for X only.
    also_Y_scores_using_all_components = np_ikpls_alg_1.transform(
        Y=Y
    )  # Also works for Y only.

    X_scores_using_five_components, Y_scores_using_five_components = (
        np_ikpls_alg_1.transform(X=X, Y=Y, n_components=5)
    )  # Shaoes are (N, 5).
    X_scores_using_five_components_only = np_ikpls_alg_1.transform(
        X=X, n_components=5
    )  # Also works for X only.
    Y_scores_using_five_components_only = np_ikpls_alg_1.transform(
        Y=Y, n_components=5
    )  # Also works for Y only.
    ...
    X_reconstructed_using_all_components, Y_reconstructed_using_all_components = (
        np_ikpls_alg_1.inverse_transform(
            T=X_scores_using_all_components, U=Y_scores_using_all_components
        )
    )
    X_reconstructed_using_all_components = np_ikpls_alg_1.inverse_transform(
        T=X_scores_using_all_components
    )  # Also works for X only
    Y_reconstructed_using_all_components = np_ikpls_alg_1.inverse_transform(
